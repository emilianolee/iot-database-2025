-- 서브쿼리 고급
-- 4-12 Orders 테이블 평균 주문금액 이하의 주문에 대해서 주문번호와 금액을 나타내시오.
...
-- 추가. 최신방법(서브쿼리에서 두가지 칼럼을 비교하는 법) - 튜플(파이썬과 동일)
-- 2010년 중반 이후부터 사용 예상
SELECT *
	FROM Orders
...
-- SELECT 서브쿼리. 스칼라값(단일행, 단일열) 한컬럼에 데이터가 1개
...
-- FROM절 서브쿼리, 인라인뷰 
-- 4-19 고객번호가 2이하인 고객의 판매액을 타나내시오(고객이름과 고객별 판매액 출력)
