-- 3-15 : 고객이 주문한도서의 총판매액을 구하시오.
SELECT SUM(saleprice) AS 총매출
	FROM Orders;

-- MySQL 문자열은 ' 홑따옴표 사용, '' 쌍따옴표는 사용불가!!
...
SELECT SUM(saleprice) AS Total			-- 합계
            ...
            , MIN(saleprice) AS Minimum		-- 최소
			, MAX(saleprice) AS Maximum	-- 최대
            , STD(saleprice) AS StandardV	-- 표준편차
	FROM Orders;

-- 그룹화 GROUP BY 키워드 사용
-- 3-19 : 고객별로 주문한 도서의 총수량과 총판매액을 구하시오.
-- GROUP BY를 사용하면 반드시 집계함수 및 GROUP BY에 포함된 컬림이 SELECT 안에 들어가 있어야 함.
...
-- 3-20 : 가격이 8000원 이상인 도서를 구매한 고객에 대하여 고객별 주문 도서의 총수량을 구하시오. 단, 2권 이상 구매한 고객에 대해서만 구하시오.
...
SELECT c.name, b.bookname
 ...
 WHERE c.custid = o.custid 
       ...
       AND b.price = 20000; -- 나머지 조건...
    
-- 외부조인 : 조건을 ㅁ나족하지 않는(일치하지 않는) 데이터도 출력이 필요할 때 사용하는 조인
